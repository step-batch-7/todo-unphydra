...
const createANewTodo = function(req, res) {
  ...
  const list = new List(title, dateOb, id);
  allList[id] = list;
  res.statusCode = 200;
  res.end();
};
...
const serveCardDetails = function(req, res) {
  sendTodoStatus(res, allList[req.query.id]);
};

const deleteAItemOfTodo = function(req, res) {
  const [todoId] = req.body.id.split('_');
...
app.get(serveCardDetails, '/cardDetails');

app.post(createANewTodo, '/takeTitle');
